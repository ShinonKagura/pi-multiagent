...
| 7.6 | L6 compat-surface | 🟡 AGENT + /agent DONE | `Agent` tool + `/agent <persona> <task>` command resolve a persona (L1), map to a single-step detached graph (pure mapper, schema-valid + `resolveDetachedGraph`-plannable), and start via the inherited `agent_team` substrate (detached, returns runId). Pending: foreground inline-result wait, `get_subagent_result`, `steer_subagent`. `Profile` tool + `/profile` are now wired (L2 resolve + L1 personas → L3 chain/parallel graph → `agent_team` start). Real-Pi smoke is 7.7. |
