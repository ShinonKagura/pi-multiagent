...
| 7.6 | L6 compat-surface | 🟡 AGENT + /agent DONE | `Agent` tool + `/agent <persona> <task>` command resolve a persona (L1), map to a single-step detached graph (pure mapper, schema-valid + `resolveDetachedGraph`-plannable), and start via the inherited `agent_team` substrate (detached, returns runId). Pending: foreground inline-result wait. `Profile`/`/profile` and `get_subagent_result`/`steer_subagent` are now wired (`get_subagent_result` → run_status; `steer_subagent` → message with auto live-step resolution). Real-Pi smoke is 7.7. |
