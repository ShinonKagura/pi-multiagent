...
function resolveStepAgent(stepId: string, spec: GraphSpec["steps"][number]["agent"], authority: GraphAuthority, library: { sources: LibrarySource[] }, libraryAgents: AgentConfig[], diagnostics: AgentDiagnostic[], context: ResolveGraphContext, skillContext: ReturnType<typeof createCallerSkillResolutionContext>, path: string): ResolvedAgent | undefined {
	...
	if (system !== undefined) {
		...
		return { id: stepId, ref: `inline:${stepId}`, name: stepId, kind: "inline", description: stepId, tools: toolAccess.tools, extensionTools: toolAccess.extensionTools, callerSkills: skills, systemPrompt, model: spec.model, fallbackModels: spec.fallbackModels, thinking: normalizeStepThinking(spec.thinking), source: "inline", filePath: undefined, sha256: undefined };
	}
...
function resolveLibraryAgent(stepId: string, spec: GraphSpec["steps"][number]["agent"] & { ref: string }, authority: GraphAuthority, library: { sources: LibrarySource[] }, libraryAgents: AgentConfig[], diagnostics: AgentDiagnostic[], context: ResolveGraphContext, skillContext: ReturnType<typeof createCallerSkillResolutionContext>, path: string): ResolvedAgent | undefined {
	...
	const skills = resolveAgentCallerSkills({ mode: context.subagentSkillMode ?? DEFAULT_SUBAGENT_SKILL_MODE, tools: toolAccess.tools, label: `step agent ${stepId}`, path: `${path}/skills`, allowProjectCode: authority.allowProjectCode, diagnostics, context: skillContext });
	if (!skills) return undefined;
	return { id: stepId, ref: agent.ref, name: agent.name, kind: "library", description: agent.description, tools: toolAccess.tools, extensionTools: toolAccess.extensionTools, callerSkills: skills, systemPrompt: agent.systemPrompt, model: spec.model ?? agent.model, fallbackModels: spec.fallbackModels ?? agent.fallbackModels, thinking: spec.thinking === undefined ? agent.thinking : normalizeStepThinking(spec.thinking), source: agent.source, filePath: agent.filePath, sha256: agent.sha256 };
}
