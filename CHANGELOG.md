...
- **FIX-6 (HIGH, found in real-Pi smoke)** — RPC child controller spawns step children with `detached: true` (non-Windows) and piped stdio, but never called `child.unref()`. Result: even after all 3 step children exited cleanly and the run reached terminal status, the parent Pi process kept its libuv event loop alive on the dead-child ChildProcess handles. Symptom: `pi -p ...` hangs at the shell prompt after the model's terminal JSON; Mark had to open a new terminal after every smoke run. Fixed with `this.child.unref?.()` immediately after spawn. Exit/close/stderr listeners still fire after `unref()`, so completion handling is unchanged.
