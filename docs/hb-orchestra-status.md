...
| L4 harness-contracts | path/scope governance enforcement: a planning-time **forbidden-path gate** (blocks a mutating step whose declared `mutationScope` targets a contract `forbiddenPath`) + **governance injection** (allowedPaths/forbiddenPaths/mutationScope/externalSideEffects appended to child prompts). Planning + prompt tier, not an OS sandbox — hard mutation confinement stays worktree isolation's job. | ✅ | `extensions/orchestra/src/harness-contracts/apply.ts` (`forbiddenPathViolation`, `buildHarnessPolicyText`), `index.ts` (`harness-policy-denied`) | `tests/orchestra-harness-apply.test.ts` (12) + real-Pi probe (block + allow) |
| L4 harness-contracts | parse the full nested-YAML PlanPacket (Lite/pointer form is enough for the v1.0 core) | ⏳ | — | — |
